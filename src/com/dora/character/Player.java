...
{	
	...
	private final int PLAYER_IMAGE_SIZE = 32;
	...
	public Player(float xOffset, float yOffset, GameState gs)
	{
		...
		SpriteSheet playerSpriteSheet = new SpriteSheet(spriteSheetImage, PLAYER_IMAGE_SIZE, PLAYER_IMAGE_SIZE);
	...
	public void render()
	{
		...
		{
			leftWalkAnim.draw(Globals.SCREEN_WIDTH/2 - this.PLAYER_IMAGE_SIZE/2, Globals.SCREEN_HEIGHT/2 - this.PLAYER_IMAGE_SIZE/2);
		}else if(playerMoveDirection.compareTo(MoveDirection.right) == 0)
		{
			rightWalkAnim.draw(Globals.SCREEN_WIDTH/2 - this.PLAYER_IMAGE_SIZE/2, Globals.SCREEN_HEIGHT/2 - this.PLAYER_IMAGE_SIZE/2);
		}else if(playerMoveDirection.compareTo(MoveDirection.up) == 0)
		{
			upWalkAnim.draw(Globals.SCREEN_WIDTH/2 - this.PLAYER_IMAGE_SIZE/2, Globals.SCREEN_HEIGHT/2 - this.PLAYER_IMAGE_SIZE/2);
		}else if(playerMoveDirection.compareTo(MoveDirection.down) == 0)
		{
			downWalkAnim.draw(Globals.SCREEN_WIDTH/2 - this.PLAYER_IMAGE_SIZE/2, Globals.SCREEN_HEIGHT/2 - this.PLAYER_IMAGE_SIZE/2);
		}else if(playerMoveDirection.compareTo(MoveDirection.stationary) == 0)
		{
			defaultImage.draw(Globals.SCREEN_WIDTH/2 - this.PLAYER_IMAGE_SIZE/2, Globals.SCREEN_HEIGHT/2 - this.PLAYER_IMAGE_SIZE/2);
		}
	}
